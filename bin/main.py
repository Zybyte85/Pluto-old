...
class Application:
    ...
    def make_task(self, priority, text, due_date, due_time, initial=False):
        ...
        if not initial:
            ...
            write_json({"text": text, "due_date": due_date, "due_time": due_time, "id": id_file}, priority)
    ...
    def handle_new_button_click(self):
        ...
        if self.dialog.ui.exec() == 0:
            ...
            self.make_task(self.dialog.ui.taskPriority.currentText(), self.dialog.ui.taskName.toPlainText(), self.dialog.ui.taskTime.time().toString(), self.dialog.ui.taskDate.date().toString())
